...
const departments = allJobs?.length ? [...new Set(allJobs.map((job) => job.department))] : [];
const workModels = allJobs?.length ? [...new Set(allJobs.map((job) => job.work_model))] : [];
...
const filteredJobs = allJobs.filter((job) => {
  ...
  const typeMatch =
    (!filters.fullTime && !filters.partTime && !filters.contract && !filters.intern) ||
  ...
  const workModelMatch =
    filters.workModels.length === 0 || filters.workModels.includes(job.work_model);
...
const isFilterApplied = () => {
  const { fullTime, partTime, contract, intern, departments, workModels } = filters;
  return (
    fullTime || partTime || contract || intern ||
    departments.length > 0 || workModels.length > 0 || search.trim() !== ""
  );
};
  ...
  <div className="space-y-6">
    {currentJobs.length > 0 ? (
      currentJobs.map((job, index) => (
        ...
        >
          <h3 className="text-xl font-semibold text-green-300">{job.title.split(" – ")[0]}</h3>
          <p className="text-sm text-green-400">{job.location} • {job.job_type}</p>
          <p className="mt-2 text-green-200">{job.description || "No description available."}</p>
...
>
  Apply Now
      ...
      <p className="text-green-400 text-center">No jobs found matching your filters.</p>
    ) : (
      <p className="text-green-400 text-center">No jobs available at the moment.</p>
    )}
  </div>
...
<div className="flex justify-center items-center space-x-2 mt-8 text-green-200">
  ...
  {[...Array(totalPages)].map((_, idx) => {
    ...
    if (
      ...
      return <span key={pageNum} className="px-2 select-none">...</span>;
    }
    return null;
  })}
...
<ContinuousImageScroll/>
  ...
  viewport={{ once: false, amount: 0.3 }}
>
        <h2 className="text-3xl font-bold mb-4">Didn't find a matching role?</h2>
        <p className="mb-6 text-green-300">Send us your resume anyway — we’re always hiring great talent.</p>
